...
/// business pattern. Runs as its own pipeline stage BEFORE Grouping
/// partitioning (see <see cref="Matching.ReconciliationEngine"/>) — named
/// rules are curated, asserted business knowledge, so they get first pick of
/// the transaction pool ahead of anything the algorithm would otherwise
/// figure out on its own.
///
/// Ships with an EMPTY default list (see <see cref="ReconciliationSettings.SpecialComboRules"/>
/// remarks for why) — this is a plain, UI-editable list; add, remove, or edit
/// rules from the Settings window if you have a known bank/R365 pairing that
/// should always be grouped together unconditionally, regardless of amount
/// or date. Column numbers below are just a reasonable starting point for a
/// newly-added rule, not a shipped default.
/// </summary>
public sealed class SpecialComboRule
{
    /// <summary>1-based Excel column number on the Bank side this rule
    /// checks, independent of <see cref="ColumnMapping.BankDescriptionColumn"/>
    ...
    /// checks, independent of <see cref="ColumnMapping.R365ReferenceColumn"/>
    /// — change it per rule as needed.</summary>
