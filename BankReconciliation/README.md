...
- All 69 xUnit tests in `BankReconciliation.Core.Tests` pass: 45 from the original hand-written suite, 9 covering `GroupingMatcher`, 6 covering `ReportExporter`, 8 integration-style tests driving the real two-worksheet `ExcelService` against actual `.xlsx` files on disk, and one full end-to-end test replaying the real-world scenario (a numeric Grouping ID that ties out, one that doesn't, one that's R365-only, a "Sysco" named-rule keyword sharing the same Grouping column, and a plain ungrouped match) through the complete Load → RunAsync → WriteResultsAndSave pipeline and back.
