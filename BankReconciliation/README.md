...
**Match ID.** Every matched transaction — one-to-one or combination — is written the same unique, sortable number on its Bank row and its R365 row(s), in a column of its own past the sheet's real data (column M on Bank, column K on R365, by default — see [Column mapping](#column-mapping)) so it can never collide with an existing column the workbook already uses for something else. Sort the Bank Transactions or R365 Transactions tab by that column in Excel and every matched set lines up together, in order, so you can visually trace a deposit to the postings that make it up. Unmatched rows are left blank in that column — a NoMatch or Manual Review row never gets a number here, even one GroupingMatcher locked as part of a one-sided or non-tying Grouping bucket. Turn this off with **Write Match ID column** in Settings.
...
- All 71 xUnit tests in `BankReconciliation.Core.Tests` pass: 45 from the original hand-written suite, 9 covering `GroupingMatcher`, 6 covering `ReportExporter`, 8 integration-style tests driving the real two-worksheet `ExcelService` against actual `.xlsx` files on disk, one full end-to-end test replaying the real-world scenario (a numeric Grouping ID that ties out, one that doesn't, one that's R365-only, a "Sysco" named-rule keyword sharing the same Grouping column, and a plain ungrouped match) through the complete Load → RunAsync → WriteResultsAndSave pipeline and back, one real performance benchmark (~12,300 transactions in under 2 seconds — see [Performance notes](#performance-notes)), and one regression test for the Matched/Unmatched double-counting bug described below.
...
- **A third, more serious bug was found from a real user's actual run** against their real workbook, diagnosed from a screenshot and an uploaded output file rather than from a test in isolation: `GroupingMatcher` gave every row it touched a real `GroupId`, even for a one-sided or non-tying bucket it was reporting as `NoMatch`/`ManualReview` — a `GroupId` is supposed to mean "the engine paired this row with something," which was untrue for those rows. Combined with a summary calculation that counted "Matched" by the `IsMatched` flag (`true` for these rows too, kept for a legitimate but unrelated reason — see the class's exclusivity remarks) instead of by status, the Summary Dashboard could show the exact same rows as both Matched and Unmatched at once, and the Excel output stamped a Match ID number onto rows whose own Notes column called them "No Match." Fixed at the source (`GroupingMatcher` now only assigns a real `GroupId` when a bucket genuinely ties out) plus in the summary/`MatchGroups` calculation (now keyed off `Status`, matching the same tri-state check the UI already used) — see `ReconciliationEngine.IsGenuinelyMatched`.
